...
const EmployeeMapView = () => {
  ...
  if (!mapData) return <div className="p-5 text-sm text-gray-600">Loading Map View...</div>;
  ...
  return (
    <div className="w-full h-full bg-gray-50" style={{ height: "calc(100vh - 3.5rem)" }}>
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm">
          <div className="flex flex-wrap items-center gap-2 text-xs text-gray-700">
            ...
            <span className="rounded-md bg-gray-100 px-2 py-1">Total: {mapData.summary?.totalLots ?? 0}</span>
            <span className="rounded-md bg-green-50 px-2 py-1 text-green-700">Available: {mapData.summary?.availableLots ?? 0}</span>
            <span className="rounded-md bg-yellow-50 px-2 py-1 text-yellow-700">Pending: {mapData.summary?.pendingLots ?? 0}</span>
            <span className="rounded-md bg-red-50 px-2 py-1 text-red-700">Sold: {mapData.summary?.soldLots ?? 0}</span>
          </div>
        </div>
      </div>
              ...
              return (
                <React.Fragment key={lot.lot_id}>
                  <Polygon
                    ...
                    pathOptions={{ color: statusColor, fillColor: statusColor, fillOpacity: 0.4, weight: 2 }}
                  >
                    <Popup>
                      <div>
                        <strong>{lot.lot_number}</strong><br />
                        Area: {lot.area_sqm} SQM<br />
                  ...
                  <Marker
                    ...
                    eventHandlers={{ click: () => handlePinClick(lot) }}
                  >
                    <Tooltip direction="top" offset={[0, -10]}>
                      <div className="text-center text-xs leading-tight">
                        ...
                        <div className="mb-1 text-[12px] font-bold" style={{ color: statusColor }}>{lot.status}</div>
                      </div>
                    </Tooltip>
                  </Marker>
                </React.Fragment>
      ...
      {/* LotOffcanvas — employee can assign client & update lot status */}
      <LotOffcanvas
