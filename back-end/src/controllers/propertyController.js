...
exports.createProperty = async (req, res) => {
	const { property_name, location, description, total_lots, status } = req.body;
    ...
    (property_name, location, description, total_lots, status)
    VALUES (?, ?, ?, ?, ?)
  `;

	try {
		const [result] = await db.query(query, [property_name, location, description || null, total_lots || 0, status || "active"]);
		res.status(201).json({ message: "Property added successfully", property_id: result.insertId });
	} catch (err) {
		res.status(500).json({ error: err.message });
	}
};
...
exports.updateProperty = async (req, res) => {
	...
	const query = `
    UPDATE properties
    SET property_name = ?, location = ?, description = ?, 
        total_lots = ?, status = ?
    WHERE property_id = ?
  `;

	try {
		const [result] = await db.query(query, [property_name, location, description, total_lots, status, id]);
...
exports.updatePropertyStatus = async (req, res) => {
	...
	try {
		const [result] = await db.query("UPDATE properties SET status = ? WHERE property_id = ?", [status, id]);
